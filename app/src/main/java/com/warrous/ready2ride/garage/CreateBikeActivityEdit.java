...
import com.warrous.ready2ride.createbike.CreateBikeContract;
import com.warrous.ready2ride.createbike.CreateBikePresenter;
...
public class CreateBikeActivityEdit extends BaseActivity implements CreateBikeContract.View{
    ...
    int brandPos=0,yearPos=0,modelPos=0;

    ImagePickerUtils imagePickerUtils;
    String imageUrl;

    int brandId=-1,yearid=-1,modelId=-1,brandYearId=-1;
    File imagePath;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        defaultBikeDetailsResponse=(DefaultBikeDetailsResponse) getIntent().getSerializableExtra("bikeDetails");
        brandId=defaultBikeDetailsResponse.getBrandId();

        mPresenter.ongetBrands(0);
        if(!TextUtils.isEmpty(defaultBikeDetailsResponse.getCycleName())){
             etBikeName.setText(defaultBikeDetailsResponse.getCycleName());
         }

        if(!TextUtils.isEmpty(defaultBikeDetailsResponse.getVIn())){
            etVin.setText(defaultBikeDetailsResponse.getVIn());
        }
        if(!TextUtils.isEmpty(String.valueOf(defaultBikeDetailsResponse.getOdometerReading()))){
            etOdoMeterTReading.setText(String.valueOf(defaultBikeDetailsResponse.getOdometerReading()));
        }
        if(!TextUtils.isEmpty(String.valueOf(defaultBikeDetailsResponse.getPurchaseDate()))){
            etPdate.setText(defaultBikeDetailsResponse.getPurchaseDate());
        }



         modelId=defaultBikeDetailsResponse.getModelId();
    ...
    public int getBrandPos(int brandId){
        int brandPos=0;
    ...
    public void setYearId(){
        yearValue=defaultBikeDetailsResponse.getYear();
        yearid=getYearid(yearValue);
        brandYearId=getbrandYearid(yearValue);

    }

    public int getYearPos(int yearid){
        int yearpos=0;

        for(int i=0;i<yearList.size();i++){
            if(yearList.get(i).getYearId()==yearid){
                yearpos=i;
            }
        }
        return yearpos;
    ...
    public int getModelPos(int modelid){
        int modelpos=0;
...
//            Log.e("ERROR",e.getMessage());
    ...
    public void bindBrandAdapter(ArrayList<Brand> brandLists){
        ...
        brandPos=getBrandPos(brandId);
        if(brandPos>0){
            spMake.setSelection(brandPos);
        }
        }
...
//        }
        ...
        int brandd=brandList.get(spMake.getSelectedItemPosition()).getBrandId();
        int yeard=yearList.get(spYear.getSelectedItemPosition()).getYearId();
        String yearame=yearList.get(spYear.getSelectedItemPosition()).getYearValue();
        int modeld=modelList.get(spModel.getSelectedItemPosition()).getModelId();

        CreateBikeRequest createBikeRequest=new CreateBikeRequest();

        createBikeRequest.setMake(brandd);
        createBikeRequest.setModel(modeld);
        createBikeRequest.setYear(yearame);
        ...
        createBikeRequest.setCycleId(defaultBikeDetailsResponse.getCycleId());
        if(TextUtils.isEmpty(etReading)){
    ...
    public void onSucessCreateBike(ArrayList<SignUpResponse> signUpResponses) {
        ActivityUtils.startActivity(this,GarageActivity.class,null);
        finish();
    }
